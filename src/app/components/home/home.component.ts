...
import { Component, OnInit, inject, PLATFORM_ID, OnDestroy, AfterViewInit, HostListener, ElementRef, ViewChild } from '@angular/core';
...
private lastScrollY = 0;
  ...
  selectedErpImage: ProjectImageItem | null = null;
  selectedErpIndex = 0;
  @ViewChild('erpCarouselTrack') erpCarouselTrack?: ElementRef<HTMLDivElement>;
  ...
  /**
   * Galería del proyecto ERP a medida (SEMI)
   */
  erpProjectImages: ProjectImageItem[] = [
    { src: '/imgs/img1.png', alt: 'ERP a medida pantalla 1', title: 'ERP - Vista 1' },
    { src: '/imgs/img2.png', alt: 'ERP a medida pantalla 2', title: 'ERP - Vista 2' },
    { src: '/imgs/img3.png', alt: 'ERP a medida pantalla 3', title: 'ERP - Vista 3' },
    { src: '/imgs/img4.png', alt: 'ERP a medida pantalla 4', title: 'ERP - Vista 4' },
    { src: '/imgs/img5.png', alt: 'ERP a medida pantalla 5', title: 'ERP - Vista 5' },
    { src: '/imgs/img6.png', alt: 'ERP a medida pantalla 6', title: 'ERP - Vista 6' }
  ];
  ...
  testimonials: TestimonialItem[] = [
    {
      text: 'El ERP a medida nos ayudó a centralizar procesos clave del negocio. El equipo de JW fue muy cercano y cumplió con lo acordado.',
      name: 'Equipo de Operaciones',
      role: 'Área de Operaciones',
      company: 'SEMI Telecomunicaciones'
    },
    {
      text: 'La Bitácora Mobile mejoró el control en campo y nos dio trazabilidad en tiempo real. Fue una solución práctica y efectiva para nuestro equipo técnico.',
...
handleNavLink(sectionId: string, event?: Event): void {
...
private animateCounters(): void {
  ...
  // ==========================================================================
  // GALERÍA ERP - PREVIEW EN MISMA PANTALLA
  // ==========================================================================

  openErpPreview(image: ProjectImageItem, index: number): void {
    this.selectedErpImage = image;
    this.selectedErpIndex = index;
  }

  closeErpPreview(): void {
    this.selectedErpImage = null;
  }

  showNextErpImage(): void {
    if (!this.erpProjectImages.length) return;
    this.selectedErpIndex = (this.selectedErpIndex + 1) % this.erpProjectImages.length;
    this.selectedErpImage = this.erpProjectImages[this.selectedErpIndex];
  }

  showPrevErpImage(): void {
    if (!this.erpProjectImages.length) return;
    this.selectedErpIndex = (this.selectedErpIndex - 1 + this.erpProjectImages.length) % this.erpProjectImages.length;
    this.selectedErpImage = this.erpProjectImages[this.selectedErpIndex];
  }

  @HostListener('document:keydown.escape')
  onEscapeKey(): void {
    if (this.selectedErpImage) {
      this.closeErpPreview();
    }
  }

  scrollErpCarousel(direction: 'left' | 'right'): void {
    const track = this.erpCarouselTrack?.nativeElement;
    if (!track) return;
